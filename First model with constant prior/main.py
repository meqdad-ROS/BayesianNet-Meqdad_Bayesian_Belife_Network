...
params2 = ['bComfort', 'ventSystem', 'thermal_isolation', 'acSystem', 'airQualityComf', 'AcousComfort', 'visualComf', 'thermalComf']
...
axes01 = az.plot_posterior(fit, var_names=params0, combine_dims={"draw"}, figsize =(50,50), textsize = 10) #filter_vars="like",
fig01 = axes01.ravel()[0].figure
fig01.savefig('output/fit_posterior_mean_plot.pdf')
plt.savefig("posterior_ mean_plot.pdf")
plt.savefig("posterior_mean_plot.png")

axes011 = az.plot_posterior(fit, var_names=params1, combine_dims={"draw"}, figsize =(50,50), textsize = 10) #filter_vars="like",
...
for i in range(1,len(params0)):
    # for j in range(0,len(params2)-1):
        ...
        axes02 = az.plot_pair(fit, var_names=[params0[i-1],params0[i]], marginals=True, combine_dims={"draw"}, figsize =(50,50), textsize = 10) # filter_vars="like", 
        fig02= axes02.ravel()[0].figure
        fig02.savefig('output/fit_pair_mean_plot_0'+ str(i-1)+'.pdf')

        axes022 = az.plot_pair(fit, var_names=[params1[i-1],params1[i]], marginals=True, combine_dims={"draw"}, figsize =(50,50), textsize = 10) # filter_vars="like", 
...
axes030 = az.plot_trace(fit,var_names=params2, divergences=None, compact=True, combined=True, figsize =(24,16)) #kind= {“trace”, “rank_bars”, “rank_vlines”},filter_vars="like",
fig030 = axes030.ravel()[0].figure
fig030.savefig('output/fit_plot.pdf')
plt.savefig("trace_plot.pdf")
plt.savefig("trace_plot.png")

axes03 = az.plot_trace(fit,var_names=params0, divergences=None, compact=True, combined=True, figsize =(24,16)) #kind= {“trace”, “rank_bars”, “rank_vlines”},filter_vars="like",
...
axes033 = az.plot_trace(fit,var_names=params1, divergences=None, compact=True, combined=True, figsize =(24,16)) #kind= {“trace”, “rank_bars”, “rank_vlines”},filter_vars="like",
...
for i in range(0, dfFullLength):
    ...
    metabolism.append(outputParameters_mu['met_a1_mu'] * outputParameters['ageTheta'][0][df['age'].iloc[i]-1] + 
                        outputParameters_mu['met_a2_mu'] * outputParameters['weightTheta'][0][df['weight'].iloc[i]-1] + #weightTransformed
                        outputParameters_mu['met_a3_mu'] * outputParameters['timeTheta'][0][df['time'].iloc[i]-1] + #timeTransformed
                        outputParameters_mu['met_a4_mu'] * outputParameters['bodyActivityTheta'][0][df['bodyActivity'].iloc[i]-1] +  
    ...
    thermal_comfort.append(outputParameters_mu['therm_a1_mu'] * metabolism[i] + 
                            ...
                            outputParameters_mu['therm_a9_mu'] * zero_inflated(df['nigaThermal'].iloc[i], outputParameters_mu['nigaTheraml_Alfa_mu']) + 
                            outputParameters_mu['therm_a10_mu'] *  outputParameters['clothLayerTheta'][0][df['clothLayer'].iloc[i]-1] +
                            outputParameters_mu['therm_const_mu'])

    illumnaation_levl.append(outputParameters_mu['light_a1_mu'] * df['lightLevel'].iloc[i] + 
                             outputParameters_mu['light_a2_mu'] * zero_inflated(df['luminaireOFF'].iloc[i], outputParameters_mu['luminaireOFFAlfa_mu']) + 
                             outputParameters_mu['light_a3_mu'] * zero_inflated(df['lightControl'].iloc[i], outputParameters_mu['lightControlAlfa_mu']) + 
                             outputParameters_mu['light_a4_mu'] * zero_inflated(df['BlindOFF'].iloc[i], outputParameters_mu['BlindOFFAlfa_mu']) * 
    ...
    visual_comfort.append(outputParameters_mu['visual_a1_mu'] * illumnaation_levl[i] + 
                           ...
                           outputParameters_mu['visual_a3_mu'] * zero_inflated(df['nigaLightQua'].iloc[i], outputParameters_mu['nigaLightAlfa_mu']) + 
    ...
    Acoustic_isolation.append(outputParameters_mu['acou_isolation_mu_mu'] * zero_inflated(df['outNoise'].iloc[i], 
                                    outputParameters_mu['outNoiseAlfa_mu'])+ outputParameters_mu['acou_isol_cons_mu'])

    #-------------------------------------------------------------------------------------------------------------------
    Acoustic_Comfort.append(outputParameters_mu['acous_a1_mu'] * zero_inflated(df['backgNoise'].iloc[i], outputParameters_mu['backgNoise_Alfa_mu']) + 
                            outputParameters_mu['acous_a2_mu'] * zero_inflated(df['nigaNoise'].iloc[i], outputParameters_mu['nigaNoise_Alfa_mu']) + 
                            ...
                            outputParameters_mu['acous_a7_mu'] * outputParameters['ageTheta'][0][df['age'].iloc[i]-1]  + 
                            outputParameters_mu['acous_cons_mu'])    
    
    
    voc_consentarion.append((df['lpg'].iloc[i] + df['alcohol'].iloc[i] + df['nitric'].iloc[i] + df['ch2o'].iloc[i] + df['methane'].iloc[i] + 
    ...
    airquailty_calc.append((voc_consentarion[i] + df['co2'].iloc[i] + df['o2'].iloc[i] +
                             df['co'].iloc[i]  + df['h2'].iloc[i] ) + outputParameters_mu['speed_factor_mu'] * df['air_speed'].iloc[i]) #
    ...
    Air_quality_Comfort.append(outputParameters_mu['a4_mu'] + 
                                ...
                                outputParameters_mu['a13_mu'] * outputParameters['bodyActivityTheta'][0][df['bodyActivity'].iloc[i]-1] + 
                                outputParameters_mu['a14_mu'] * zero_inflated(df['odorUsually'].iloc[i], outputParameters_mu['odorUsuAlfa_mu'])+ 
                                outputParameters_mu['a15_mu'] * zero_inflated(df['sleepy'].iloc[i], outputParameters_mu['sleepyAlfa_mu']) + 
                                outputParameters_mu['a16_mu'] * zero_inflated(df['nigaAir'].iloc[i], outputParameters_mu['nigaAirAlfa_mu']) + 
                                outputParameters_mu['a17_mu'] * zero_inflated(df['ac_control'].iloc[i], outputParameters_mu['acControlAlfa_mu']))
    ...
    tc_probability.append(stats.norm.cdf(thermal_comfort[i], loc= outputParameters_mu['thermalComf_mu_mu'], scale=outputParameters_mu['thermalComf_std_mu']))
    vc_probability.append(stats.norm.cdf(visual_comfort[i], loc= outputParameters_mu['visualComf_mu_mu'], scale=outputParameters_mu['visualComf_std_mu']))
    acc_probability.append(stats.norm.cdf(Acoustic_Comfort[i], loc= outputParameters_mu['AcousComfort_mu'], scale=outputParameters_mu['AcousComfort_std_mu']))
    aic_probability.append(stats.norm.cdf(Air_quality_Comfort[i], loc= outputParameters_mu['airQualityComf_mu_mu'], scale=outputParameters_mu['airQualityComf_std_mu']))

    bcPercentage.append(stats.norm.cdf(Building_Comfort[i], loc= outputParameters_mu['bComfort_mu_mu'], scale=outputParameters_mu['bComfort_std_mu']))
    hvacPercentage.append(stats.norm.cdf(HVACsys_condition[i], loc= outputParameters_mu['acSystem_mu'], scale= outputParameters_mu['acSystem_std_mu']))
    tiPercentage.append(stats.norm.cdf(thermal_isolation[i], loc= outputParameters_mu['thermal_isolation_mu'], scale= outputParameters_mu['thermal_isolation_std_mu']))
    ventSysPercentage.append(stats.norm.cdf(Ventilation_Quality[i], loc= outputParameters_mu['ventSystem_mu'], scale=outputParameters_mu['ventSystem_std_mu']))
    aiPercentage.append(stats.norm.cdf(Acoustic_isolation[i], loc= outputParameters_mu['acou_isolation_mu'], scale= outputParameters_mu['acou_isolation_std_mu']))

    
    
predectionData = pd.DataFrame(np.array([thermal_comfort, tc_probability, visual_comfort, vc_probability, Acoustic_Comfort, 
