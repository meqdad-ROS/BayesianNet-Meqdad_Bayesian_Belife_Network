...
parameters {
    ...
    real <lower=0, upper=0.9999> nigaLightAlfa; // propability of answering yes there is a nigative effect of ighting quality; 
    ...
    //----------------------------------------------
    real <lower=0, upper=0.9999> nigaNoise_Alfa; // propability of answering yes noise and sound have nigative effect; 
    real <lower=0, upper=0.9999> outNoiseAlfa; // probability of there is an exterior sound or noise 
    ...
    real <lower=0, upper=0.9999> backgNoise_Alfa; //probability of saying yes to the above question.
    ...
    real <lower=0, upper=0.9999> odorAlfa; // propability of answering yes smell odor;
    ...
    real <lower=0, upper=0.9999> acControlAlfa; // propability of answering yes in the space; 
    real <lower=0, upper=0.9999> sleepyAlfa; // propability of answering yes in the space;
    real <lower=0, upper=0.9999> odorUsuAlfa; // propability of answering yes smell odor usually;
    ...
    real <lower=0, upper=10> acou_isolation_std; // standard deviation of measured wall quality Isolation.
    ...
    real <lower=0 , upper=5> metabolism_std;
    real <lower=0 , upper=5> latentLightlevel_sigma;
}
//--------------------------------------------------------------------------------------------------------------------------------
//******************************************************************************************************************************//
//--------------------------------------------------------------------------------------------------------------------------------
transformed parameters{
    ...
    vector[N] acou_isolation_mu; // mean of measured wall quality Isolation. 
    

    metabolism = met_a1 * ageTransformed + met_a2 * weightTransformed + met_a3 * timeTransformed 
                        + met_a4* bodyActivityTransformed + met_const;
    //--------------------------------------------- -2- --------------------------------------------
    thermalComf = therm_a1 * metabolism + therm_a2 * to_vector(air_speed) + therm_a3 * to_vector(Temp) + therm_a4 * to_vector(Rtemp) 
    ...
    // //------------------------------------------ -3- -----------------------------------------------    
    latentLightlevel = light_a1 * to_vector(lightLevel) + light_a2 * luminaireOFFTransformed + light_a3 * lightControlTransformed + 
                                    light_a4 * alfa_DayL + light_cons + latentLightlevel_sigma;
    // //---------------------------------------------- -4- -------------------------------------------
    visualComf = visual_a1 * latentLightlevel + visual_a2 * to_vector(lightColor) + visual_a3 * nigaLightQuaTransformed + visual_a4 * to_vector(colorComf) + 
    ...
    // //-----------------------------------------------------------------------------------------    
    acou_isolation_mu = acoisola_alfa* outNoiseTransformed + acoisola_const;
    acou_isolation  = acou_isolation_mu .* to_vector(outNoise) + acou_isol_cons + acou_isolation_std;
    // //--------------------------------------------- -5- --------------------------------------------
    AcousComfort = acous_a1 * backgNoiseTransformed + acous_a2 * nigaNoiseTransformed + acous_a3 * acou_isolation 
    ...
    // //-----------------------------------------------------------------------------------------
    vocCons = (to_vector(lpg) + to_vector(alcohol) + to_vector(nitric) + to_vector(ch2o) + 
                            to_vector(methane) + to_vector(voc)) .* odorTransformed + vocCons_std;
    airQuality = vocCons + co2_mu + o2_mu + co_mu + h2_mu + speed_factor* speed_loc + airQuality_std;  // predected mean of Air quality
    // //------------------------------------------------- -6- ----------------------------------------
    airQualityComf  = a4 + a1*airQuality + a2 * to_vector(airSpeedNow) + a3 * to_vector(vent) + a13 * bodyActivityTransformed + a14 *  odorUsuallyTransformed 
                            + a15 *  sleepyTransformed + a16 *  nigaAirTransformed + a17 * ac_controlTransformed + airQualityComf_std;  
    //---------------------------------------------Intermediate nodes--------------------------------------------
    bComfort = bui_a1 * airQualityComf + bui_a2 * AcousComfort + bui_a3 * visualComf + bui_a4 * thermalComf + bui_const + bComfort_std;
    //-----------------------------------------------------------------------------------------
    acSystem = aSys_a1 * airQuality  + aSys_a2 * to_vector(air_speed)  + aSys_a3 * bComfort + aSys_cons + acSystem_std;
    //---------------------------------------------Output nodes--------------------------------------------
    thermal_isolation = b1* acSystem  + b2 * to_vector(Rtemp) + b3 + thermal_isolation_std;
    //-----------------------------------------------------------------------------------------
    ventSystem  = c + backgNoiseTransformed + c1*airQualityComf  + c2 * to_vector(air_speed) + ventSystem_std;
    //-----------------------------------------------------------------------------------------
    //-----------------------------------------------------------------------------------------
    //-----------------------------------------------------------------------------------------
    artLights  = latentLightlevel + artLights_std;
...
model{
    ...
    //-----------------------------------------------------------------------------------------
    airQuality_std ~ normal(4,1); 
    // //-----------------------------------------------------------------------------------------
    thermalComf_std ~ normal(6,2);
    visualComf_std ~ normal(5, 2); 
    acou_isolation_std ~ normal(5, 2);
    AcousComfort_std ~ normal(5, 2);
    // //-----------------------------------------------------------------------------------------
    bComfort_std ~ normal(5, 2);
    // //-----------------------------------------------------------------------------------------
    acSystem_std ~ normal(5,2);
    //----------------------------------------------------------------------------------------------
    //----------------------------------------------------------------------------------------------
    //----------------------------------------------------------------------------------------------
    // met_a1 ~ std_normal();
    ...
    // therm_a8 ~ std_normal();
    // therm_a9 ~ std_normal();
    // therm_a10 ~ std_normal();
    // therm_const ~ normal(0,40);
    // // //------------------------------------------ -3- -----------------------------------------------    
    // light_a1 ~ std_normal();
    // light_a2 ~ std_normal();
    // light_a3 ~ std_normal();
    // light_a4 ~ std_normal();
    // light_cons ~ normal(0,4);
    // // //---------------------------------------------- -4- -------------------------------------------
    // visual_a1 ~ std_normal();
    // visual_a2 ~ std_normal();
    // visual_a3 ~ std_normal();
    // visual_a4 ~ std_normal();
    // visual_a5 ~ std_normal();
    // visual_a6 ~ std_normal();
    // visual_a7 ~ std_normal(); 
    // visual_cons ~ std_normal();
    // // //-----------------------------------------------------------------------------------------    
    // acoisola_alfa ~ std_normal();
    // acoisola_const ~ std_normal();
    // acou_isol_cons ~ std_normal();
    // // //--------------------------------------------- -5- --------------------------------------------
    // acous_a1 ~ std_normal(); 
    // acous_a2 ~ std_normal(); 
    // acous_a3 ~ std_normal();
    ...
    // bui_a3 ~ std_normal(); 
    // bui_a4 ~ std_normal();
    // bui_const ~ normal(0,4);
    // //-----------------------------------------------------------------------------------------
    // aSys_a1 ~ std_normal();
    // aSys_a2 ~ std_normal();
    // aSys_a3 ~ std_normal();
    // aSys_cons ~ normal(0,4);
    // //---------------------------------------------Output nodes--------------------------------------------
    // b1 ~ std_normal();
    // b2 ~ std_normal();
    // b3 ~ std_normal();
    ...
    // c1 ~ std_normal();
    // c2 ~ std_normal();
    // // //--------------------------------------------------------calculate suggested Prior---------------------------------
    // //--------------------------------------------------------calculate suggested Prior---------------------------------
    metabolism_std ~ std_normal();
    latentLightlevel_sigma ~ std_normal();
    //-----------------------------------------------------------------------------------------
    //------------------------------------------calculate Posterior-----------------------------------------------
}
//--------------------------------------------------------------------------------------------------------------------------------
...
mean_acou_isolation_mu = mean(acou_isolation_mu);
